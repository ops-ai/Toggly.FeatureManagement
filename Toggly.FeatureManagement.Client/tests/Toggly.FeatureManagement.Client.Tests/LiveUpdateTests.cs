...
{
    [Fact]
    public async Task DebouncesUpdatesRefreshesSigningKeysAndIgnoresLegacyPayloads()
    {
        ...
        Assert.Contains("sdk=dotnet-client", updates.Uri!.Query);
        foreach (var message in new[] { "{\"type\":\"evaluated\",\"defs\":{\"on\":false}}", "{\"type\":\"sync\",\"unchanged\":true}", "{\"type\":\"flags-updated\",\"etag\":\"\\\"v1\\\"\"}" })
            await updates.Messages.Writer.WriteAsync(message);
        await Task.Delay(400);
        Assert.Equal(1, calls);
        for (var i = 0; i < 4; i++)
            await updates.Messages.Writer.WriteAsync("{\"type\":\"flags-updated\",\"etag\":\"v2\"}");
        await WaitUntil(() => calls == 2);
        Assert.True(client.IsEnabled("on"));
        await updates.Messages.Writer.WriteAsync("invalid");
        await updates.Messages.Writer.WriteAsync("{\"type\":\"signing-key-updated\"}");
        await WaitUntil(() => keys == 2 && calls == 3);
    }
    [Theory]
    [InlineData("update")]
    ...
    public async Task NotificationsWithoutRevisionBypassConditionalCache(string notification)
    {
        ...
        await client.InitializeAsync();
        await updates.Messages.Writer.WriteAsync(notification);
        await refreshed.Task.WaitAsync(TimeSpan.FromSeconds(3));
        await WaitUntil(() => client.IsEnabled("on"));
    }
    [Fact]
    public async Task RevisionNotificationPinsFetchUntilHttpConfirmsIt()
    {
        using var fixture = new SignedFixture();
        var updates = new Updates();
        var requests = new List<(string Query, bool Conditional)>();
        using var http = new HttpClient(new Handler((request, ct) =>
        {
            ...
            response.Headers.TryAddWithoutValidation("ETag", requests.Count == 1 ? "\"v1\"" : "\"v2\"");
            return Task.FromResult(response);
        }));
        await using var client = new TogglyClient(new()
        {
        ...
        Assert.Contains("rev=v2", requests[1].Query);
        Assert.False(requests[1].Conditional);
    }
    [Fact]
    public async Task NewNotificationCancelsAndDrainsTheSupersededRefresh()
    {
        using var fixture = new SignedFixture();
        var updates = new Updates();
        var calls = 0;
        ...
        await client.InitializeAsync();
        await updates.Messages.Writer.WriteAsync("update");
        await started.Task.WaitAsync(TimeSpan.FromSeconds(3));
        await updates.Messages.Writer.WriteAsync("flags-updated");
    ...
    [Fact]
    public async Task DisposalDrainsRefreshEvenWhenTransportDelaysCancellation()
    {
        using var fixture = new SignedFixture();
        var started = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var release = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    [Fact]
    public async Task SubscriberFailuresDoNotSkipLaterHandlersAndHandlersCanBeRemoved()
    {
        using var fixture = new SignedFixture();
        ...
        client.Changed += throwingChange;
        client.Changed += change;
        client.Error += throwingError;
        client.Error += error;
        await client.InitializeAsync();
    ...
    [Fact]
    public async Task PollingContinuesWithoutLiveUpdatesAndDisposalStopsIt()
    {
        using var fixture = new SignedFixture();
        var calls = 0;
    ...
    [Fact]
    public async Task WebSocketFramesAreCombinedAndConnectionIsDisposed()
    {
        ...
        var source = new WebSocketUpdates((uri, ct) => Task.FromResult<WebSocket>(socket));
        var messages = new List<string>();
        await foreach (var message in source.ListenAsync(new("wss://example.com")))
            messages.Add(message);
        Assert.Equal("{\"type\":\"sync\"}", Assert.Single(messages));
        Assert.True(socket.Disposed);
    }
    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public async Task InvalidOrOversizedWebSocketFramesAreRejected(bool binary)
    {
        var frames = binary ? new[] { (new byte[1], true, WebSocketMessageType.Binary) } : Enumerable.Range(0, 9).Select(_ => (new byte[8192], false, WebSocketMessageType.Text)).ToArray();
        var socket = new FakeSocket(frames);
        var source = new WebSocketUpdates((uri, ct) => Task.FromResult<WebSocket>(socket));
        await Assert.ThrowsAsync<WebSocketException>(async () => { await foreach (var message in source.ListenAsync(new("wss://example.com"))) { } });
        Assert.True(socket.Disposed);
    }
}
internal sealed class FakeSocket(IEnumerable<(byte[] Data, bool End, WebSocketMessageType Type)> frames) : WebSocket
{
    ...
    public override Task CloseAsync(WebSocketCloseStatus status, string? description, CancellationToken ct) => Task.CompletedTask; public override Task CloseOutputAsync(WebSocketCloseStatus status, string? description, CancellationToken ct) => Task.CompletedTask; public override void Dispose() => Disposed = true;
