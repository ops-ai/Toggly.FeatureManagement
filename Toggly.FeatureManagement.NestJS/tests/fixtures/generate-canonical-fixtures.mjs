...
async function generate(identity) {
  ...
  const coordinates = Buffer.concat([Buffer.from(jwk.x, 'base64url'), Buffer.from(jwk.y, 'base64url')]);
  const kid = Buffer.from(await subtle.digest('SHA-1', coordinates)).toString('hex').toUpperCase() + 'ES256';
  const defs = JSON.stringify([
    ...
    rule('groups', 'Targeting', { 'Audience.Groups:0:Name': 'staff', 'Audience.Groups:0:RolloutPercentage': 100 }),
    { ...rule('vip', 'ContextProperty', { Property: 'Vip', Operator: 'eq', Value: 'true', ValueType: 'boolean' }), contextKind: 'Order' },
  ], null, 2);
  ...
  return { defs, timestamp, signature: Buffer.from(signature).toString('base64'), kid, jwks: { keys: [{ ...jwk, alg: 'ES256', kid }] } };
}
writeFileSync(new URL('./canonical-worker.json', import.meta.url), JSON.stringify({ initial: await generate('alice'), rotated: await generate('bob') }, null, 2) + '\n');
