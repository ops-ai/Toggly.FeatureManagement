...
describe('transport and lifecycle', () => {
  it('reports network and invalid body failures with defaults', async () => {
    for (const response of [
      ...
      const c = createClient({
        ...
        verifySignatures: false,
        fetch: fetcher,
        flagDefaults: { safe: false },
      });
      await c.refresh();
      expect(c.state().error).toBeInstanceOf(Error);
      expect(c.flags()).toEqual({ safe: false });
      c.dispose();
    }
  });
  it('rejects unsigned definitions by default', async () => {
    const c = createClient({
      appKey: 'test',
      fetch: vi.fn().mockResolvedValue(new Response('{"unsafe":true}')),
    });
    await c.refresh();
    expect(c.evaluate(['unsafe'])).toBe(false);
    expect(c.state().error).toBeDefined();
    c.dispose();
  });
  it('sends conditional revisions and retains a verified in-memory snapshot on 304/failure', async () => {
    const f = vi
      .fn()
      .mockResolvedValueOnce(new Response('{"on":true}', { headers: { ETag: 'rev1' } }))
  ...
  it('applies entity and local gates, notifies subscribers and isolates options', async () => {
    ...
    expect(
      c.evaluate(['ExpressCheckout'], 'all', false, {
        kind: 'Order',
        key: '1',
        attributes: { Vip: true },
      }),
    ...
    allowed = true;
    c.notifyLocalGatesChanged();
    expect(c.evaluate(['on'])).toBe(true);
    expect(() =>
      c.setLocalGates([
        { id: '1', flagKeys: ['on'], isEnabled: () => true },
        { id: '2', flagKeys: ['on'], isEnabled: () => true },
      ]),
    ).toThrow();
    unsubscribe();
    listener.mockClear();
  ...
  it('copies targeting, clears it explicitly and does no work after dispose', async () => {
    ...
    await c.setContext({ identity: '', groups: [], claims: {} });
    expect(c.context().groups).toEqual([]);
    c.dispose();
    await c.setContext({ identity: 'ignored' });
    expect(c.context().identity).toBe('');
    c.start();
  });
  it('cancels pending requests and never emits after disposal', async () => {
    let resolve!: (value: Response) => void;
    const f = vi.fn(() => new Promise<Response>((r) => (resolve = r)));
    const c = createClient({ appKey: 'test', fetch: f, verifySignatures: false });
    const listener = vi.fn();
  ...
  it('enforces request timeout', async () => {
    ...
    const c = createClient({ appKey: 'test', fetch: f, connectTimeout: 20 });
    const pending = c.refresh();
    await vi.advanceTimersByTimeAsync(25);
    await pending;
    expect(c.state().error?.message).toBe('aborted');
    c.dispose();
    vi.useRealTimers();
  });
  it('handles live invalidation, revision pins, reconnects and cleanup', async () => {
    ...
    const f = vi.fn(() =>
      Promise.resolve(new Response('{"on":true}', { headers: { ETag: 'old' } })),
    );
    const c = createClient({
      appKey: 'test',
      verifySignatures: false,
    ...
    expect(String(s.url)).toContain('rev=old');
    for (const data of [
      'bad',
      '{"type":"ping"}',
      '{"type":"sync","unchanged":true}',
      '{"type":"update","etag":"old"}',
    ])
      s.onmessage({ data });
    await vi.advanceTimersByTimeAsync(300);
    expect(f).toHaveBeenCalledTimes(1);
    s.onmessage({ data: '{"type":"flags-updated","etag":"new"}' });
    await vi.advanceTimersByTimeAsync(300);
    expect(String(f.mock.calls[1][0])).toContain('rev=new');
    expect(f.mock.calls[1][1].headers['If-None-Match']).toBeUndefined();
    s.onmessage({ data: '{"type":"signing-key-updated"}' });
    await vi.advanceTimersByTimeAsync(300);
    s.onmessage({ data: '{"type":"sync","etag":"another"}' });
    await vi.advanceTimersByTimeAsync(300);
    s.onclose();
    await vi.advanceTimersByTimeAsync(5000);
    expect(Socket.all).toHaveLength(2);
    await vi.advanceTimersByTimeAsync(10000);
    expect(f.mock.calls.length).toBeGreaterThan(4);
    c.dispose();
    expect(Socket.all[1].close).toHaveBeenCalled();
    expect(Socket.all[1].onclose).toBeNull();
    vi.unstubAllGlobals();
    vi.useRealTimers();
  });
  it.each(['update', 'flags-updated', '{"type":"update"}', '{"type":"flags-updated"}'])(
    'fetches unconditionally after legacy invalidation %s',
    async (data) => {
      vi.useFakeTimers();
      class Socket {
      ...
      const f = vi.fn((_url, init) =>
        Promise.resolve(
          init.headers['If-None-Match']
            ? new Response(null, { status: 304 })
            : new Response(JSON.stringify({ on: f.mock.calls.length > 1 }), {
                headers: { ETag: 'old' },
              }),
        ),
      );
  ...
  it('retries unavailable sockets and permits HTTP-only runtimes', async () => {
    ...
    const c = createClient({ appKey: 'test', refreshInterval: 0 });
    c.start();
    ...
    const d = createClient({ appKey: 'test', refreshInterval: 0 });
    d.start();
    expect(d.state().error).toBeUndefined();
    d.dispose();
    vi.unstubAllGlobals();
    vi.useRealTimers();
  });
  ...
  it('verifies signed startup, rejects tampering and isolates cache by context', async () => {
    ...
    const signature = Buffer.from(
      await webcrypto.subtle.sign(
        ...
        pair.privateKey,
        await webcrypto.subtle.digest('SHA-256', new TextEncoder().encode(`${raw}|${timestamp}`)),
      ),
    ).toString('base64');
    ...
    let offline = false;
    const f = vi.fn((url: any) =>
      offline
        ? Promise.reject(new Error('offline'))
        : String(url).includes('.well-known')
          ? Promise.resolve(
              new Response(JSON.stringify({ keys: [{ ...publicKey, kid, alg: 'ES256' }] })),
            )
          : Promise.resolve(new Response(envelope)),
    );
    const c = createClient({ appKey: 'test', identity: 'alice', fetch: f, storage });
    await c.refresh();
    expect(c.state().error).toBeUndefined();
    expect(c.evaluate(['signed'])).toBe(true);
    expect(map.size).toBe(2);
    c.dispose();
    offline = true;
    const cached = createClient({ appKey: 'test', identity: 'alice', fetch: f, storage });
    await cached.refresh();
    expect(cached.evaluate(['signed'])).toBe(true);
    cached.dispose();
    const bob = createClient({ appKey: 'test', identity: 'bob', fetch: f, storage });
    await bob.refresh();
    expect(bob.evaluate(['signed'])).toBe(false);
    bob.dispose();
    for (const [key, value] of map) map.set(key, value.replace('true', 'false'));
    const tampered = createClient({ appKey: 'test', identity: 'alice', fetch: f, storage });
    await tampered.refresh();
    expect(tampered.flags()).toEqual({});
    tampered.dispose();
    vi.unstubAllGlobals();
  });
...
it.each(['visible', 'unexposed'])(
  'retains verified browser state and revision after a signed malformed %s gate',
  async (keyName) => {
    const { webcrypto } = await import('node:crypto');
    const { computeKid } = await import('@ops-ai/toggly-signed-defs');
    const pair = await webcrypto.subtle.generateKey({ name: 'ECDSA', namedCurve: 'P-256' }, true, [
    ...
    async function sign(defs: unknown) {
    ...
    const f = vi.fn(async (url, init) => {
      if (String(url).includes('.well-known'))
        return new Response(JSON.stringify({ keys: [{ ...key, kid, alg: 'ES256' }] }));
      calls.push(init);
      return new Response(
        await sign(
          malformed
            ? { visible: true, [keyName]: { requirement: 'all', rules: [{ property: 'Vip' }] } }
            : { visible: true },
        ),
        { headers: { ETag: malformed ? 'bad-rev' : 'good-rev' } },
      );
    });
    const c = createClient({ appKey: 'front', fetch: f, expose: ['visible'] });
    try {
      await c.refresh();
      expect(c.flags()).toEqual({ visible: true });
      malformed = true;
      await c.refresh();
      expect(c.flags()).toEqual({ visible: true });
      expect(c.state().error).toBeInstanceOf(Error);
      expect(
        c.evaluate(['visible'], 'all', false, {
          kind: 'Order',
          key: '1',
          attributes: { Vip: true },
        }),
      ...
      await c.refresh();
      expect(calls.at(-1).headers['If-None-Match']).toBe('good-rev');
    } finally {
      c.dispose();
    }
  },
