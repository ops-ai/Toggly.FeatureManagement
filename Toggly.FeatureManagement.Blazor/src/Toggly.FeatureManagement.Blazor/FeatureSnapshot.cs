...
public sealed class FeatureSnapshot
{
    ...
    /// <summary>Returns false when any requested key was not explicitly allowed into this snapshot.</summary>
    ...
    )
    {
        ...
        result = false;
        if (requested.Any(k => !Values.ContainsKey(k)))
            return false;
        result =
            requested.Length == 0
            || (
                requirement == Requirement.Any
                    ? requested.Any(k => Values[k])
                    : requested.All(k => Values[k])
            );
        if (negate)
            result = !result;
        return true;
