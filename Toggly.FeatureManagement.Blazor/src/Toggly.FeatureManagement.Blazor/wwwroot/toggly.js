...
export async function verify(definitions, timestamp, signature, kid, jwksJson) {
  try {
    ...
    const digest = await crypto.subtle.digest(
      "SHA-256",
      new TextEncoder().encode(definitions + "|" + timestamp),
    );
...
// Storage contains signed envelopes and previously accepted public keys. The portable
// client checks context, age, configured pins and signatures again before evaluation.
export function load(contextKey) {
  try {
    return JSON.parse(localStorage.getItem("toggly:" + contextKey));
  } catch {
    return null;
  }
}
export function save(contextKey, snapshot) {
  try {
    localStorage.setItem("toggly:" + contextKey, JSON.stringify(snapshot));
  } catch {
