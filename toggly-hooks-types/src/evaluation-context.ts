 ...
 */
export function normalizeEvaluationClaims(
 ...
 */
export function appendEvaluationContext(
  url: URL,
  context: TogglyEvaluationContext | undefined,
  mode: EvaluationUrlMode = 'evaluated'
): void {
  if (!context) {
    return;
  }
 ...
 */
export function evaluationContextCacheKey(context: TogglyEvaluationContext | undefined): string {
  if (!context) {
    return '';
  }
