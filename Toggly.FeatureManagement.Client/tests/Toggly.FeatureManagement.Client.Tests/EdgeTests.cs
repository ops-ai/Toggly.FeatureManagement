...
{
    [Fact]
    public async Task AllAnyDefaultsAndThrowingPrerequisites()
    {
        using var http = new HttpClient();
        await using var client = new TogglyClient(new()
        {
            Defaults = new Dictionary<string, bool> { { "on", true }, { "blocked", true } },
            LocalGates = new Dictionary<string, Func<bool>> { { "blocked", () => throw new InvalidOperationException("device unavailable") } }
        }, http, new Es256SignatureVerifier());
        var errors = 0;
        client.Error += (_, _) => errors++;
        await Task.WhenAll(client.InitializeAsync(), client.InitializeAsync());
        Assert.True(client.Evaluate([]));
        Assert.False(client.Evaluate([], negate: true));
        Assert.True(client.Evaluate(["missing", "on"], Requirement.Any));
        Assert.False(client.Evaluate(["missing", "on"]));
        Assert.True(client.IsEnabled("on"));
        Assert.False(client.IsEnabled("blocked"));
    ...
    public void InvalidConfigurationIsRejected()
    {
        ...
        foreach (var options in new[] { new TogglyClientOptions { RefreshInterval = TimeSpan.Zero }, new() { MaximumSignatureAge = TimeSpan.Zero }, new() { BaseUri = new("http://example.com") }, new() { WebSocketBaseUri = new("ws://example.com") } })
            Assert.Throws<ArgumentException>(() => new TogglyClient(options, http, verifier));
    }
    [Fact]
    public async Task MalformedUnsignedBodiesAndDuplicateEnvelopeFieldsNeverApply()
    {
        ...
        var errors = 0;
        client.Error += (_, _) => errors++;
        foreach (var payload in new[] { "{}", "{\"on\":true}", fixture.Envelope("{\"on\":42}"), fixture.Envelope("{\"on\":true}").Replace("{\"defs\":", "{\"defs\":{},\"defs\":") })
        {
            body = payload;
    ...
    [Fact]
    public async Task SnapshotContextMismatchAndCorruptionCannotLeakOtherUserFlags()
    {
        using var fixture = new SignedFixture();
        var store = new CorruptStore(fixture.Envelope("{\"on\":true}"));
    ...
    [Fact]
    public async Task SigningKeyRotationRecoversWithoutWebSocketNotification()
    {
    ...
    [Fact]
    public async Task DefaultWebSocketConnectionFailuresAreReportedAndDisposed()
    {
