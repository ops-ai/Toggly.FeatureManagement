...
with tempfile.TemporaryDirectory(prefix='toggly-packed-consumer-') as temporary:
    ...
    candidate = ET.SubElement(mappings, 'packageSource', key='candidate')
    ET.SubElement(candidate, 'package', pattern='Toggly.*')
    public = ET.SubElement(mappings, 'packageSource', key='nuget')
