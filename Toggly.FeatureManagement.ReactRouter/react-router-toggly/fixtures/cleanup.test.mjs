...
import { cleanupOwned, closeBrowser, closeServer, runOwnedCommand, stopChild } from './template/owned-resources.mjs'
...
if (process.argv[2] === '--scenario') {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'router-cleanup-control-'))
  ...
  for (const scenario of ['launch', 'assertion', 'close-reject', 'close-hang', 'cleanup', 'logging']) {
    test(`owned cleanup preserves failure and reaps real resources: ${scenario}`, { timeout: 10000 }, async () => {
      const root = mkdtempSync(join(tmpdir(), 'router-cleanup-test-'))
      try {
