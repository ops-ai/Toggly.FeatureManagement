...
public sealed partial class TogglyClient : IAsyncDisposable
{
    ...
    /// <summary>True after initialization completes, including safe fallback to defaults when no verified state is available.</summary>
    public bool IsReady
    {
        get; private set;
    }
    ...
    /// <summary>Raised after accepted definitions or user context changes. Subscribers may run on background threads.</summary>
    ...
    /// <summary>Reports transport, verification, storage or subscriber errors without discarding last-known-good state.</summary>
    ...
    /// <summary>Creates one frontend session. The caller owns HttpClient and supplies the host cryptography and storage boundaries.</summary>
    public TogglyClient(TogglyClientOptions options, HttpClient http, ISignatureVerifier verifier, ISnapshotStore? store = null, IUpdateSource? updates = null)
    {
        ...
        if (options.RefreshInterval <= TimeSpan.Zero || options.MaximumSignatureAge <= TimeSpan.Zero || !options.BaseUri.IsAbsoluteUri || options.BaseUri.Scheme != "https" || options.WebSocketBaseUri.Scheme != "wss")
            throw new ArgumentException("Use HTTPS/WSS endpoints and positive intervals.", nameof(options));
        this.options = options with
        {
            Defaults = new Dictionary<string, bool>(options.Defaults),
            ...
            lock (stateLock)
            {
                ...
                if (!string.IsNullOrWhiteSpace(options.AppKey))
                {
                    ...
                    if (options.EnableLiveUpdates)
                        liveTask = LiveAsync(lifetime.Token);
                }
            }
        }
    ...
    /// <summary>Evaluates a feature and optional entity against verified state, then applies any local prerequisite.</summary>
    ...
    {
        var keys = featureKeys.ToArray();
        var result = keys.Length == 0 || (requirement == Requirement.Any ? keys.Any(k => IsEnabled(k, entity)) : keys.All(k => IsEnabled(k, entity)));
    ...
    /// <summary>Serializes refreshes and retains verified state when transport or signature validation fails.</summary>
    public async Task RefreshAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                keys = await RestoreSnapshotAsync(key, version, keys, ct).ConfigureAwait(false) ?? keys;
    ...
    private async Task<string?> RestoreSnapshotAsync(string key, long version, string? keys, CancellationToken ct)
    {
        if (store is null || timestamp != 0)
            return null;
        try
        {
            var snapshot = await store.LoadAsync(key, ct).ConfigureAwait(false);
            if (snapshot is null || snapshot.ContextKey != key || snapshot.FormatVersion is not (1 or 2))
                return null;
            // Caller pins are authoritative. Legacy records require keys already trusted by the host.
            var snapshotKeys = options.TrustedJwks ?? keys ?? (snapshot.FormatVersion == 2 ? snapshot.TrustedJwks : null);
            if (snapshotKeys is null)
                return null;
            var acceptedKeys = await AcceptAsync(snapshot.Envelope, snapshot.Revision, version, snapshotKeys, ct, false).ConfigureAwait(false);
            lock (stateLock)
            {
                if (acceptedKeys is null || disposed || version != generation)
                    return null;
                jwks = acceptedKeys;
                return acceptedKeys;
            }
    ...
    private HttpRequestMessage CreateRequest(EvaluationContext current)
    {
        ...
        if (pinnedRevision is not null)
            query.Add("rev=" + Uri.EscapeDataString(pinnedRevision));
        ...
        if (conditionalRevision is not null)
            request.Headers.TryAddWithoutValidation("If-None-Match", conditionalRevision);
    ...
    private async Task FetchDefinitionsAsync(EvaluationContext current, string key, long version, string keys, CancellationToken ct)
    {
        ...
        var acceptedKeys = await AcceptAsync(body, nextRevision, version, keys, ct).ConfigureAwait(false);
        if (acceptedKeys is not null && store is not null && IsCurrent(version))
            await store.SaveAsync(key, new(key, body, nextRevision)
            {
                FormatVersion = 2,
                TrustedJwks = acceptedKeys
            }, ct).ConfigureAwait(false);
    }

    private async Task<string?> FetchKeysAsync(long version, CancellationToken ct)
    {
        if (options.TrustedJwks is not null)
            return IsCurrent(version) ? options.TrustedJwks : null;
    ...
    private async Task<string?> AcceptAsync(string body, string? rev, long version, string keys, CancellationToken ct, bool allowKeyFetch = true)
    {
        if (!IsCurrent(version))
            return null;
        ...
        if (root.EnumerateObject().Select(p => p.Name).Distinct(StringComparer.Ordinal).Count() != root.EnumerateObject().Count())
            throw new JsonException("Duplicate envelope properties.");
        ...
        {
            // Only a network response may refresh endpoint keys. Cached bytes must verify
            // independently, and explicit host trust must never be replaced by endpoint keys.
            if (!allowKeyFetch || options.TrustedJwks is not null)
                throw new CryptographicException("Invalid signature.");
            var freshKeys = await FetchKeysAsync(version, ct).ConfigureAwait(false);
            if (freshKeys is null)
                return null;
            if (!await verifier.VerifyAsync(defs.GetRawText(), time, signature, kid, freshKeys, ct).ConfigureAwait(false))
                throw new CryptographicException("Invalid signature.");
            keys = freshKeys;
        }
        var next = defs.EnumerateObject().ToDictionary(p => p.Name, p => p.Value.Clone(), StringComparer.Ordinal);
        if (next.Values.Any(v => v.ValueKind is not (JsonValueKind.True or JsonValueKind.False or JsonValueKind.Object)))
            throw new JsonException("Invalid definition.");
        lock (stateLock)
        {
            if (disposed || version != generation)
                return null;
    ...
    private void Report(Exception ex)
    {
        ...
        foreach (var handler in handlers?.GetInvocationList().Cast<EventHandler<Exception>>() ?? [])
        {
            ...
            {
                /* Consumer errors must not kill refresh or prevent other subscribers running. */
            }
        }
    }
    ...
    private async Task PollAsync(CancellationToken ct)
    {
        try
        {
            while (true)
            {
                await Task.Delay(options.RefreshInterval, ct).ConfigureAwait(false);
                await RefreshAsync(ct).ConfigureAwait(false);
            }
        }
        catch (OperationCanceledException) when (ct.IsCancellationRequested)
        {
            /* Normal lifetime shutdown. */
        }
    }

    /// <summary>Cancels and drains background work and caller-owned refreshes before releasing session resources.</summary>
