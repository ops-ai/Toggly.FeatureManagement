...
(async () => {
  ...
  try {
    ...
    await waitFor(() => packets.length > hiddenBefore); assert.equal(packets.at(-1).headers['content-encoding'], 'gzip');
    ...
    console.log(JSON.stringify({wasm:true, crossOrigin:true, packets:packets.length, preflights:preflights.length, lifecycle:'pagehide and disposal verified'}));
