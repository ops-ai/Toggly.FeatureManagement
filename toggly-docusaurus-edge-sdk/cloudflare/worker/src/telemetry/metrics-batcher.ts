...
export interface MetricStatHttpPayload {
  appKey: string;
  environment: string;
  /** ISO-8601 for gateway HTTPS JSON path. */
  time: string;
  stats: Array<{
    metric: string;
    feature?: string;
...
export class MetricsBatcher {
  ...
  isEmpty(): boolean {
    return (
      this.measures.size === 0 && this.counters.size === 0 && this.observations.length === 0
    );
  }
  ...
  buildAndReset(): MetricStatHttpPayload | null {
    if (this.isEmpty()) {
      return null;
    }
    ...
    const payload: MetricStatHttpPayload = {
      appKey: this.appKey,
