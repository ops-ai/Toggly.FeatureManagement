...
export function createTogglyHandle(options: ServerOptions): Handle {
  ...
  return async ({ event, resolve }) => {
    ...
    const snapshot = async (): Promise<TogglySnapshot> => {
      ...
      try {
        ...
        if (result.notModified)
          throw new Error('Unexpected 304 without a verified request snapshot');
    ...
    event.locals.toggly = {
      ...
      gate: (keys, gate = {}) =>
        options.client.evaluateFeatureGate(
          keys,
          gate.requirement,
          gate.negate,
          context,
...
function scoped(event: RequestEvent): RequestToggly {
  if (!event.locals.toggly)
    throw new Error('Install createTogglyHandle in hooks.server.ts before using Toggly helpers');
  return event.locals.toggly;
}
/** Return only this snapshot from +layout.server.ts; never serialize the Node client. */
export function loadToggly(event: RequestEvent): Promise<TogglySnapshot> {
  return scoped(event).snapshot();
}
/** Server load/action guard. Feature gating supplements your existing authentication/authorization. */
export async function requireFeature(
  event: RequestEvent,
  keys: string | string[],
  options: Omit<GateOptions, 'defaultValue'> = {},
): Promise<void> {
  if (!(await scoped(event).gate(typeof keys === 'string' ? [keys] : keys, options)))
    error(404, 'Feature unavailable');
}
