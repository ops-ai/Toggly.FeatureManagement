...
public sealed class BrowserModule(IJSRuntime js) : IAsyncDisposable
{
    ...
    public Task<IJSObjectReference> GetAsync() =>
        module ??= js.InvokeAsync<IJSObjectReference>(
                "import",
                "./_content/Toggly.FeatureManagement.Blazor/toggly.js"
            )
            .AsTask();
...
/// <summary>Verifies canonical signed definitions using browser WebCrypto.</summary>
public sealed class BrowserSignatureVerifier(BrowserModule module) : ISignatureVerifier
{
    public async ValueTask<bool> VerifyAsync(
        ...
        await (await module.GetAsync()).InvokeAsync<bool>(
            "verify",
            cancellationToken,
            definitionsJson,
            timestamp,
...
/// <summary>Stores signed snapshots in durable localStorage; the portable core re-verifies before use.</summary>
public sealed class BrowserSnapshotStore(BrowserModule module) : ISnapshotStore
{
    public async ValueTask<ClientSnapshot?> LoadAsync(
        string contextKey,
        CancellationToken cancellationToken = default
    ) =>
        await (await module.GetAsync()).InvokeAsync<ClientSnapshot?>(
            "load",
            cancellationToken,
            contextKey
        );
    ...
    public async ValueTask SaveAsync(
        ...
        await (await module.GetAsync()).InvokeVoidAsync(
            "save",
            cancellationToken,
            contextKey,
            snapshot
