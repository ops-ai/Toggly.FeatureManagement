...
export function createBrowserTelemetry(config: TogglyConfig) {
  ...
  const getReporter = () => {
    if (!reporter) {
      reporter = createTelemetryReporter({appKey: config.appKey, environment: config.environment,
  ...
  function event(entry: Entry) {
    ...
    for (const value of entries) {
      if (value.kind === 'feature' && value.key === entry.key) variants++;
  ...
  function feature(key: string, variant: string, index: number) {
    const values = [0,0,0]; values[index] = 1;
    event({key, variant, kind: 'feature', values});
  }
  const api: FrontendTelemetry = {
    ...
    incrementCounter: (key, value = 1) => event({key, kind: 'counter', values: [value]}),
    setGauge: (key, value) => event({key, kind: 'gauge', values: [value]}),
  ...
  return {
    ...
    dispose() {
      disposed = true; staged.clear(); detach?.(); reporter?.dispose();
    },
  };
}
