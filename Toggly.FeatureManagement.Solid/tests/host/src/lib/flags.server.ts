...
const backend = createTogglyClient({
  ...
  featureDefaults: { BetaDashboard: true },
...
export async function scope(identity: string, request = getRequestEvent()!.request) {
  ...
  return createTogglyRequest({
    ...
    frontend: {
      ...
      baseURI: process.env.TOGGLY_BASE_URL,
      expose: ['BetaDashboard', 'LiveFeature', 'ExpressCheckout'],
      flagDefaults: { BetaDashboard: false },
      onError: () => {
        if (identity === 'observer-throw') {
          throw new Error('observer throw');
        }
        if (identity === 'observer-reject') {
