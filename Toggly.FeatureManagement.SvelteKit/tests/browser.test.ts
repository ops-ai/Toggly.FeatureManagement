...
const initial = {
  ...
  context: { identity: 'alice', groups: ['team'], claims: { role: 'admin' } },
  expose: ['on'],
};
  ...
  const fetcher = vi.fn(async (input: any) => {
    ...
    return new Response(
      String(input).endsWith('/.well-known/jwks')
        ...
        : envelope({ on: enabled, secret: true }),
    );
  });
  vi.stubGlobal('fetch', fetcher);
  const t = createToggly(initial, { appKey: 'frontend', refreshInterval: 50, onError: errors });
  await t.start();
  await vi.advanceTimersByTimeAsync(1);
  await vi.waitFor(() => expect(t.isEnabled('on')).toBe(true));
  expect(get(t).definitions).toEqual({ on: true });
  const url = new URL(fetcher.mock.calls[0][0]);
  expect(url.searchParams.get('u')).toBe('alice');
  expect(url.searchParams.get('g')).toBe('team');
  enabled = false;
  Socket.all[0].onmessage({ data: '{"type":"flags-updated"}' });
  await vi.waitFor(() => expect(t.isEnabled('on')).toBe(false));
  online = false;
...
it('switches contexts before old requests finish and never publishes after disposal', async () => {
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: any) => {
      ...
      if (new URL(input).searchParams.get('u') === 'alice')
        return new Promise<Response>((r) => (release = r));
      return new Response(envelope({ on: false }));
    }),
  );
  const t = createToggly(initial, { appKey: 'frontend', refreshInterval: 0 });
  await t.start();
  await settled();
  t.update({ ...initial, context: { identity: 'bob' }, definitions: { on: false } });
  await settled();
...
it('handles keyless defaults, cancellation during import and no live socket mode', async () => {
  socket();
  const noKey = connectBrowser(initial, {}, () => {});
  noKey();
  ...
  const stop = connectBrowser(
    initial,
    { appKey: 'frontend', enableLiveUpdates: false, refreshInterval: 0, onError: errors },
    () => {},
  );
  await settled();
...
it.each(['unsigned', 'invalid', 'wrongkid', 'expired'])(
  'rejects %s transport data without replacing the SSR branch',
  async (mode) => {
    socket();
    const errors = vi.fn();
    vi.stubGlobal(
          ...
          new Response(
            String(input).endsWith('/.well-known/jwks')
              ...
              : mode === 'unsigned'
                ...
                : mode === 'invalid'
                  ? envelope([])
                  : envelope({ on: true }, mode === 'expired' ? 1 : undefined),
          ),
      ),
    );
    const t = createToggly(initial, {
      ...
      onError: errors,
      allowedKeyIds: mode === 'wrongkid' ? ['wrong'] : undefined,
...
it.each(['update', 'flags-updated', '{"type":"update"}', '{"type":"flags-updated"}'])(
  'refreshes unconditionally for an invalidation without an etag: %s',
  async (message) => {
    vi.useFakeTimers();
    const Socket = socket();
    let enabled = true;
    ...
    await vi.waitFor(() => expect(t.isEnabled('on')).toBe(true));
    enabled = false;
    Socket.all[0].onmessage({ data: message });
    await vi.waitFor(() => expect(t.isEnabled('on')).toBe(false));
    const requests = fetcher.mock.calls.filter(
      (call) => !String(call[0]).endsWith('/.well-known/jwks'),
    );
    expect(requests).toHaveLength(2);
    expect((requests[1] as any)[1]?.cache).toBe('no-store');
    expect(new Headers((requests[1] as any)[1]?.headers).has('if-none-match')).toBe(false);
    t.dispose();
  },
);
it('uses verified conditional polling, ignores unchanged sync, pins only new revisions and reloads JWKS on key update', async () => {
  ...
  const fetcher = vi.fn(async (input: any, init: any) => {
    if (String(input).endsWith('/.well-known/jwks'))
      return new Response(JSON.stringify({ keys: [jwk] }));
    return new Headers(init?.headers).get('if-none-match') === revision
      ? new Response(null, { status: 304 })
      : new Response(envelope({ on: true }), { headers: { ETag: revision } });
  });
  vi.stubGlobal('fetch', fetcher);
  const t = createToggly(initial, {
    appKey: 'frontend',
  ...
  expect(
    fetcher.mock.calls.some((call) => new Headers(call[1]?.headers).get('if-none-match') === 'r1'),
  ).toBe(true);
  for (const data of [
    new Uint8Array(),
    'broken',
    ...
    '{"type":"sync","etag":"r1"}',
    '{"type":"flags-updated","etag":"r1"}',
  ])
    Socket.all[0].onmessage({ data });
  revision = 'r2';
  Socket.all[0].onmessage({ data: '{"type":"flags-updated","etag":"r2"}' });
  await vi.advanceTimersByTimeAsync(350);
  await vi.waitFor(() =>
    expect(
      fetcher.mock.calls.some((call) => new URL(call[0]).searchParams.get('rev') === 'r2'),
    ).toBe(true),
  );
  const pinned = fetcher.mock.calls.find(
  ...
  expect(new URL(Socket.all[1].url).searchParams.get('rev')).toBe('r2');
  t.dispose();
});
it('rejects an initial 304 and reports timeout cancellation without changing defaults', async () => {
  ...
  let stop = connectBrowser(
    initial,
    { appKey: 'frontend', timeout: 10, refreshInterval: 0, onError: errors },
    () => {},
  );
  await vi.waitFor(() => expect(errors).toHaveBeenCalled());
  stop();
...
it('backs off failed sockets, recovers without browser socket support, and cancels all timers during pending requests', async () => {
  ...
  const stop = connectBrowser(initial, { appKey: 'frontend', refreshInterval: 0 }, () => {});
  expect(calls).toBe(1);
  await vi.advanceTimersByTimeAsync(5000);
  expect(calls).toBe(2);
  ...
  const withoutSocket = connectBrowser(
    initial,
    { appKey: 'frontend', refreshInterval: 0 },
    () => {},
  );
  withoutSocket();
  vi.stubGlobal(
  ...
  const pending = connectBrowser(initial, { appKey: 'frontend', refreshInterval: 0 }, () => {});
  pending();
  expect(vi.getTimerCount()).toBe(0);
});
it.each(['throw', 'reject'])(
  'retains verified browser state and cleans up when an error observer fails: %s',
  async (mode) => {
    vi.useFakeTimers();
    const Socket = socket();
    let online = true;
...
it.each(['throw', 'reject'])(
  'retries failed WebSocket construction despite an error observer failure: %s',
  async (mode) => {
    vi.useFakeTimers();
    vi.stubGlobal('window', {});
    let attempts = 0;
