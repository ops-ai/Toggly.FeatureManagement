...
test('Blazor packages have manifest versions, changelog and release inventory', () => {
  assert.ok(family);
  assert.deepEqual(family.packages.map(p => p.id), ['Toggly.FeatureManagement.Blazor', 'Toggly.FeatureManagement.Blazor.Server']);
  ...
  for (const pkg of family.packages) {
    ...
    if (pkg.id === 'Toggly.FeatureManagement.Blazor') assert.doesNotMatch(project, /Toggly.FeatureManagement\" Version=|Client.Desktop|FrameworkReference/);
  }
});
test('Blazor packed artifacts meet existing NuGet metadata contract', () => {
  if (!process.env.NUGET_BLAZOR_PACK_DIR) return;
  const result = verifyPackedNupkgs({ inventory: {...inventory, ...family}, packDir: process.env.NUGET_BLAZOR_PACK_DIR });
