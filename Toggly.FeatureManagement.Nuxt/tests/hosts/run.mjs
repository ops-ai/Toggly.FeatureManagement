import { mkdtemp, cp, writeFile, readFile, mkdir, readdir } from 'node:fs/promises'
...
try {
  ...
  // the collector can prove the SDK emits no identity metadata.
  ...
  assert.deepEqual(Object.keys(compact.payload).sort(), ['e', 'f', 'k', 'm'])
