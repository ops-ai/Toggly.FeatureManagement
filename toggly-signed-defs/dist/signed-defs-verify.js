...
var freshness_2 = require("./freshness");
...
async function sha1HexUpper(bytes) {
    const digest = await (0, webcrypto_1.getSubtleCrypto)().digest('SHA-1', bytes.slice());
    return Array.from(new Uint8Array(digest))
        .map((b) => b.toString(16).padStart(2, '0').toUpperCase())
        .join('');
}
async function sha256Bytes(data) {
    const digest = await (0, webcrypto_1.getSubtleCrypto)().digest('SHA-256', data.slice());
...
async function verifySignedDefinitions(defsRaw, envelope, jwks, allowedKids, freshness) {
    ...
    const subtle = (0, webcrypto_1.getSubtleCrypto)();
    const cryptoKey = await subtle.importKey('jwk', {
        kty: matching.kty ?? 'EC',
    ...
    const isValid = await subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, cryptoKey, p1363, firstDigest);
    if (!isValid) {
