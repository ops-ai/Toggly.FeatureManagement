...
function isCompleteDefinitions(value: unknown): value is EvaluatedDefinitions {
  return isRecord(value) && Object.values(value).every(definition =>
    typeof definition === 'boolean' || (isRecord(definition)
      && (definition.requirement === 'all' || definition.requirement === 'any')
      && Array.isArray(definition.rules)
      && definition.rules.every(rule => isRecord(rule)
        && typeof rule.property === 'string'
        && typeof rule.op === 'string' && operators.has(rule.op.toLowerCase())
        && typeof rule.value === 'string'
        && (rule.type === undefined || (typeof rule.type === 'string' && valueTypes.has(rule.type))))));
}
/** Select public keys without changing the shared entity-gate evaluator. */
export function selectDefinitions(definitions: unknown, expose?: readonly string[]): EvaluatedDefinitions {
