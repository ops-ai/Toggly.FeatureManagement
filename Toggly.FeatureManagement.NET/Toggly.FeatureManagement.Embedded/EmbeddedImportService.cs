...
public sealed class EmbeddedImportService
{
    ...
    private static bool FeaturesEqual(CatalogFeature left, CatalogFeature right) =>
        string.Equals(left.Key, right.Key, StringComparison.Ordinal) && string.Equals(left.Name, right.Name, StringComparison.Ordinal) && string.Equals(left.Description, right.Description, StringComparison.Ordinal) && string.Equals(left.Category, right.Category, StringComparison.Ordinal) && left.Enabled == right.Enabled && left.RequirementType == right.RequirementType && left.ContextRequirementType == right.ContextRequirementType && string.Equals(left.ContextKind, right.ContextKind, StringComparison.Ordinal) && left.Tags.OrderBy(tag => tag, StringComparer.Ordinal).SequenceEqual(right.Tags.OrderBy(tag => tag, StringComparer.Ordinal), StringComparer.Ordinal) && left.Rules.Count == right.Rules.Count && left.Rules.Zip(right.Rules, (first, second) => string.Equals(first.Name, second.Name, StringComparison.Ordinal) && first.Parameters.OrderBy(pair => pair.Key, StringComparer.Ordinal).SequenceEqual(second.Parameters.OrderBy(pair => pair.Key, StringComparer.Ordinal))).All(equal => equal);
