...
public class ProviderTests : BlazorTestContext
{
    ...
    {
        var authentication = AddAuthorization();
        authentication.SetAuthorized("alice");
        authentication.SetRoles("vip");
        authentication.SetClaims(new Claim("role", "admin"));
        var session = new FakeSession();
        Services.AddSingleton<IFeatureSession>(session);
        var cut = Render<FeatureProvider>(p => p.AddChildContent("content"));
        Assert.Equal(1, session.Initializations);
        Assert.Equal("alice", session.Contexts.Last().Identity);
        Assert.Contains("vip", session.Contexts.Last().Groups!);
        authentication.SetNotAuthorized();
        cut.WaitForAssertion(() => Assert.Null(session.Contexts.Last().Identity));
        var count = session.Contexts.Count;
        await DisposeComponentsAsync();
        authentication.SetAuthorized("bob");
        Assert.Equal(count, session.Contexts.Count);
    }

    [Fact]
    public void NoAuthenticationServiceWorksForOfflineStaticHosts()
    {
        Services.Remove(Services.First(x => x.ServiceType == typeof(AuthenticationStateProvider)));
        var session = new FakeSession();
        Services.AddSingleton<IFeatureSession>(session);
        Render<FeatureProvider>();
        Assert.Equal(1, session.Initializations);
        Assert.Empty(session.Contexts);
    }

    [Fact]
    public void AuthenticationMappingPrefersStableIdentifierAndAnonymousClearsEverything()
    {
        ...
        var context = FeatureProvider.UserContext(user);
        Assert.Equal("42", context.Identity);
        Assert.Equal("first", context.Claims!["role"]);
        Assert.Null(FeatureProvider.UserContext(new()).Identity);
    }

    [Fact]
    public async Task HydrationPersistsOnlyExplicitPublicFlagsAndClearsOnContextChange()
    {
        ...
        session.Notify();
        cut.WaitForAssertion(() => Assert.Equal("loading", cut.Markup));
        session.Value = true;
        state.TriggerOnPersisting();
        Assert.True(state.TryTake<Dictionary<string, bool>>("toggly-public-flags", out var values));
        Assert.Single(values!);
        Assert.True(values!["public"]);
        await DisposeComponentsAsync();
        Assert.Equal(0, session.Subscriptions);
    }

    [Fact]
    public async Task OlderAuthenticationCompletionDoesNotOverwriteNewUserOrInitializeDisposedProvider()
    {
        var authentication = new DeferredAuthentication();
        Services.AddSingleton<AuthenticationStateProvider>(authentication);
        var session = new FakeSession();
        Services.AddSingleton<IFeatureSession>(session);
        var cut = Render<FeatureProvider>();
        var old = new TaskCompletionSource<AuthenticationState>(
        ...
        old.SetResult(Authenticated("alice"));
    ...
    {
        public TaskCompletionSource<AuthenticationState> Initial = new(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
    ...
    [Fact]
    public void EmptySnapshotStaysLoadingAndBooleanCompositionMatchesCore()
    {
        AddBunitPersistentComponentState();
        Services.AddSingleton<IFeatureSession>(new FakeSession());
        var cut = Render<FeatureHydration>(p =>
            p.Add(x => x.StateKey, "other")
                .AddChildContent<Feature>(f =>
                    f.Add(x => x.Key, "missing").Add(x => x.Loading, "loading")
                )
        );
        ...
        Assert.True(snapshot.TryEvaluate(["a", "b"], Requirement.Any, false, out var result));
        Assert.True(result);
        snapshot.TryEvaluate(["a", "b"], Requirement.All, true, out result);
        Assert.True(result);
        snapshot.TryEvaluate([], Requirement.Any, false, out result);
        Assert.True(result);
    }
}
