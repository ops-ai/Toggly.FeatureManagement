...
export async function withResources(work) {
 const actions=[],errors=[];let result
 try {result=await work(cleanup=>actions.push(cleanup))} catch(error){errors.push(error)}
 for(const action of actions.reverse())try{await bounded(action,'owned cleanup',15000)}catch(error){errors.push(error)}
 if(errors.length)throw new AggregateError(errors,'Host verification/cleanup failed',{cause:errors[0]})
 return result
}
export async function closeServer(server) {
 if(!server.listening)return
 await bounded(()=>new Promise((resolve,reject)=>{server.close(error=>error?reject(error):resolve());server.closeAllConnections()}),'HTTP close')
}
export async function stopChild(child,group=false) {
 ...
 if(group&&child.pid&&process.platform!=='win32') {try{process.kill(-child.pid,'SIGKILL')}catch(error){if(error.code!=='ESRCH')throw error}}
...
export async function runOwned(command,args,options,timeout=300000) {
 return withResources(async own=>{
  ...
  await bounded(()=>new Promise((resolve,reject)=>{child.once('error',reject);child.once('close',code=>code===0?resolve():reject(Error(`${command} exited ${code}`)))}),command,timeout)
 })
}
export async function launchBrowser(chromium,own) {
 // Playwright Browser has no process accessor; own the launched BrowserServer.
 const server=await chromium.launchServer({headless:true})
 own(()=>stopChild(server.process()))
