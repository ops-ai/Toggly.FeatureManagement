...
try {
  ...
  for (const name of files.filter((n) => n.endsWith('.js'))) {
    ...
    assert.doesNotMatch(
      text,
      /backend-private-key-sentinel|toggly-node-core|node:crypto|node:fs|backend-only-sentinel/,
    );
  }
  ...
  console.log(
    'PASS packed SolidStart SSR, signed hydration, concurrent guarded actions, navigation, entity gates, live invalidation, invalid signature and malformed map retention, isolated throwing/rejecting observers, offline retention, disposal and browser secret/import scan',
  );
} finally {
  await browser?.close();
  server?.kill('SIGTERM');
