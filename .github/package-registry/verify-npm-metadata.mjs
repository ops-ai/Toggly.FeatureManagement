...
/**
 * A caret range over a strictly valid semver version.
 *
 * This is the official semver grammar with a `^` prefix, anchored end to end.
 * A looser pattern would accept trailing junk like `^1.2.3 garbage`, a second
 * comparator that reintroduces the pin, or malformed versions like `^1.2.3-..`.
 * The grammar is inlined rather than pulled from the `semver` package because
 * this script runs in CI with no install step.
 */
const CARET_RANGE =
