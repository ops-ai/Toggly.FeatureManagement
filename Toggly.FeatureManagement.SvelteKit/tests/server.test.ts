...
const event = (identity = 'alice') =>
  ({
    locals: {},
    request: new Request('http://localhost/', {
      headers: { 'user-agent': 'Chrome', 'accept-language': 'en-US', 'cf-ipcountry': 'US' },
    }),
...
describe('request scope', () => {
  it('isolates concurrent requests, copies context once, and gates actions', async () => {
    ...
    await client.init();
    client.state.definitions.set('beta', {
      featureKey: 'beta',
    ...
    const handle = createTogglyHandle({
      ...
      context: (e) => {
        contextCalls++;
        return { identity: (e as any).identity };
      },
      frontend: { expose: ['fallback'], featureDefaults: { fallback: true, secret: true } },
    });
    const results = await Promise.all(
      ['alice', 'bob'].map(async (identity) =>
        handle({
  ...
  it('requires hooks before helpers', async () => {
    expect(() => loadToggly(event())).toThrow('hooks.server');
  });
});
...
it('verifies frontend snapshot once, allowlists keys, preserves entity rules and keeps private context private', async () => {
  const client = createTogglyClient({ featureDefaults: { on: true } });
  await client.init();
  const entity = {
    requirement: 'all',
  ...
  const fetcher = vi.fn(async (input: any) =>
    String(input).endsWith('/.well-known/jwks')
      ...
      : new Response(envelope({ on: true, Order: entity, private: true })),
  );
  ...
  const h = createTogglyHandle({
    client,
    context: () => supplied,
    clientContext: (_e, c) => ({ identity: c.identity, groups: c.groups }),
    frontend: {
      appKey: 'frontend',
      expose: ['on', 'Order'],
      allowedKeyIds: [jwk.kid],
      maxSignatureAgeSeconds: 60,
    },
  });
  const e = event();
  await h({ event: e, resolve: async () => new Response() } as any);
  supplied.identity = 'changed';
  supplied.groups.push('later');
  ...
  a.definitions.on = false;
  expect(b.definitions.on).toBe(true);
  expect(fetcher).toHaveBeenCalledTimes(2);
  expect(new URL(String(fetcher.mock.calls[0][0])).searchParams.get('u')).toBe('alice');
  expect((fetcher.mock.calls[0] as any)[1]?.headers).toEqual({
    'User-Agent': 'Chrome',
...
it.each(['unsigned', 'expired', 'key', 'network', 'status', 'invalid'])(
  'fails closed to explicit exposed defaults: %s',
  async (mode) => {
    const onError = vi.fn();
    vi.stubGlobal(
      'fetch',
    ...
    const handle = createTogglyHandle({
      ...
      frontend: {
        ...
        timeout: 20,
        expose: ['fallback'],
        featureDefaults: { fallback: true, private: true },
        onError,
        allowedKeyIds: mode === 'key' ? ['wrong'] : undefined,
        maxSignatureAgeSeconds: 60,
      },
    });
    const e = event();
    await handle({ event: e, resolve: async () => new Response() } as any);
    expect((await loadToggly(e)).definitions).toEqual({ fallback: true });
    expect(onError).toHaveBeenCalledOnce();
    await client.close();
  },
);
it('binds entities and all/any/negate to the same server context', async () => {
  const client = createTogglyClient({ featureDefaults: { on: true, off: false } });
  await client.init();
  client.state.definitions.set('Order', {
    featureKey: 'Order',
    contextKind: 'Order',
    filters: [
      { name: 'ContextProperty', parameters: { Property: 'Vip', Operator: 'eq', Value: 'true' } },
    ],
  });
  const e = event();
  await createTogglyHandle({ client, frontend: { expose: [] } })({
    event: e,
  ...
  expect(
    await e.locals.toggly.isEnabled('Order', {
      entity: { kind: 'Order', key: '1', attributes: { Vip: true } },
    }),
  ).toBe(true);
  expect(await e.locals.toggly.isEnabled('Order')).toBe(false);
  await client.close();
});
it('captures absent headers, supports array guards, handles 304 and errors without callbacks', async () => {
  ...
  for (const status of [304, 503]) {
    ...
    const e = event();
    e.request = new Request('http://localhost/');
    await createTogglyHandle({
      client,
      frontend: { appKey: 'frontend', expose: ['on'], featureDefaults: { on: true } },
    })({ event: e, resolve: async () => new Response() } as any);
    expect((await loadToggly(e)).definitions.on).toBe(true);
    await requireFeature(e, ['on']);
  }
  await client.close();
});
it.each(['throw', 'reject'])(
  'preserves exposed server defaults when the error observer fails: %s',
  async (mode) => {
    const client = createTogglyClient();
    await client.init();
    vi.stubGlobal(
