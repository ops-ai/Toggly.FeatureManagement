...
public sealed partial class TogglyClient
{
    ...
    private Uri UpdateUri()
    {
        ...
        if (rev is not null)
            path += "&rev=" + Uri.EscapeDataString(rev);
    ...
    private async Task ListenForUpdatesAsync(Action received, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                received();
                if (!InvalidateFromMessage(message))
                    continue;
    ...
    private bool InvalidateFromMessage(string message)
    {
        try
        {
            if (message is "update" or "flags-updated")
                return Invalidate("update", null, false);
    ...
    {
        if (type is not ("sync" or "update" or "flags-updated" or "signing-key-updated"))
            return false;
        lock (stateLock)
        {
            if (type == "sync" && unchanged)
                return false;
            if (type != "signing-key-updated" && etag is not null && etag == revision)
                return false;
