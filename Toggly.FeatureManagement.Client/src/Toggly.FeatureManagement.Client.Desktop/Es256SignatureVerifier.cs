...
/// <summary>Native ES256 verification of the Worker wire protocol, with canonical public-key identifiers.</summary>
public sealed class Es256SignatureVerifier : ISignatureVerifier
{
    ...
    public ValueTask<bool> VerifyAsync(string definitionsJson, long timestamp, string signature, string keyId, string jwksJson, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var key = keys[0];
            if (key.GetProperty("kty").GetString() != "EC" || key.GetProperty("crv").GetString() != "P-256" || key.GetProperty("alg").GetString() != "ES256")
                return ValueTask.FromResult(false);
            var x = Decode(key.GetProperty("x").GetString()!);
            var y = Decode(key.GetProperty("y").GetString()!);
            if (x.Length != 32 || y.Length != 32 || KeyIdentifier(x, y) != keyId)
                return ValueTask.FromResult(false);
            using var ecdsa = ECDsa.Create(new ECParameters { Curve = ECCurve.NamedCurves.nistP256, Q = new ECPoint { X = x, Y = y } });
            // Worker signing prehashes the exact definitions JSON and timestamp; WebCrypto
            // applies SHA-256 again during ECDSA signing. Never reserialize these bytes.
        ...
        catch (Exception ex) when (ex is JsonException or CryptographicException or FormatException or InvalidOperationException or KeyNotFoundException)
        {
            return ValueTask.FromResult(false);
        }
    }
...
/// <summary>Convenience construction for native hosts with optional durable file storage.</summary>
