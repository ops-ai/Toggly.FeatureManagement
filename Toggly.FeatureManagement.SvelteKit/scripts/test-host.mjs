...
const run=(command,args,cwd,env={})=>new Promise((resolve,reject)=>{
 const child=spawn(command,args,{cwd,env:{...process.env,...env},stdio:['ignore','pipe','pipe']});let output='';
...
try {
 ...
 await run('npm',['install','--no-save','--package-lock=false',join(temporary,'ops-ai-toggly-sveltekit-0.1.0.tgz'),...shared],host);
 await run('npm',['run','check'],host);
 const built=await run('npm',['run','build'],host);
 if(built.includes('externalized for browser compatibility'))throw new Error('A Node module leaked into the browser build');
 async function inspect(directory){for(const entry of await readdir(directory,{withFileTypes:true})){const path=join(directory,entry.name);if(entry.isDirectory())await inspect(path);else if(entry.name.endsWith('.js')){const body=await readFile(path,'utf8');if(/backend-private-fixture|server-secret|toggly-node-core|node:crypto|node:module|@grpc|definitions-signed/.test(body))throw new Error(`Server secret/import leaked: ${path}`);}}}
 await inspect(join(host,'.svelte-kit/output/client'));
 const boundary=spawn(process.execPath,['--conditions=browser','--input-type=module','-e',"await import('@ops-ai/toggly-sveltekit/server')"],{cwd:host,stdio:'ignore'});
 if((await once(boundary,'close'))[0]===0)throw new Error('Server export resolved under browser conditions');
 ...
 server=createServer(async(req,res)=>{
  ...
  state.requests.push({path:url.pathname,user:url.searchParams.get('u'),revision:req.headers['if-none-match']??null,pin:url.searchParams.get('rev'),claims:url.searchParams.get('claim.role'),groups:url.searchParams.getAll('g')});
  const defs=backend?[
   {featureKey:'on',filters:[{name:'Targeting',parameters:{'Audience.Users:0':'alice'}}]},
 ...
 sockets=new WebSocketServer({server});sockets.on('connection',socket=>{state.connections++;socket.on('close',()=>state.closes++);});
 ...
 app=spawn(process.execPath,['build'],{cwd:host,env:{...process.env,PORT:String(port),HOST:'127.0.0.1',ORIGIN:origin,TOGGLY_APP_KEY:'backend-private-fixture',TOGGLY_BASE_URI:definitions},stdio:['ignore','pipe','pipe']});app.stdout.pipe(process.stdout);app.stderr.pipe(process.stderr);
 for(let count=0;count<100;count++){try{await fetch(origin+'/server');break;}catch{if(count===99)throw new Error('Host did not start');await new Promise(resolve=>setTimeout(resolve,100));}}
 await writeFile(join(temporary,'playwright.config.mjs'),`export default ${JSON.stringify({testDir:join(root,'tests/host-browser'),workers:1,use:{baseURL:origin},reporter:'line'})};`);
