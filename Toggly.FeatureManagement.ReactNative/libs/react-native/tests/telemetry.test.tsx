...
it('owns real Core telemetry across rerenders, AppState and opposite app/environment replacement', async () => {
  ...
  expect(packets).toEqual([{ k: 'old', e: 'Old', f: { on: { enabled: [1] } } }]);
  ...
  expect(packets[1]).toEqual({ k: 'old', e: 'Old', f: { checkout: { enabled: [0, 1, 1] } }, m: { orders: 2, cart: 3.5 } });
  ...
  await act(async () => { await api.flushTelemetry(); });
  expect(packets[2]).toEqual({ k: 'new', e: 'New', f: { on: { disabled: [1] } } });
