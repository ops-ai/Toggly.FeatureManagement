...
public sealed class FileSnapshotStore(string directory) : ISnapshotStore
{
    ...
    private string FilePath(string key)
    {
        if (key.Length != 64 || key.Any(c => !Uri.IsHexDigit(c)))
            throw new ArgumentException("Expected SHA256 context key.", nameof(key));
    ...
    public async ValueTask<ClientSnapshot?> LoadAsync(string contextKey, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<ClientSnapshot>(await File.ReadAllTextAsync(path, cancellationToken).ConfigureAwait(false));
        }
    ...
    public async ValueTask SaveAsync(string contextKey, ClientSnapshot snapshot, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await File.WriteAllTextAsync(temporary, JsonSerializer.Serialize(snapshot), cancellationToken).ConfigureAwait(false);
