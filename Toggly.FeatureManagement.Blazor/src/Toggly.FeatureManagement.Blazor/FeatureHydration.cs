...
public sealed class FeatureHydration : ComponentBase, IDisposable
{
    ...
    public IReadOnlyList<string> PublicKeys { get; set; } = [];

    [Parameter]
    public string StateKey { get; set; } = "toggly-public-flags";
    ...
    {
        if (
            State.TryTakeFromJson<Dictionary<string, bool>>(StateKey, out var values)
            && values is not null
        )
    ...
    private async Task PersistAsync()
    {
        ...
        foreach (var key in PublicKeys.Distinct())
            values[key] = await Session.EvaluateAsync([key]);
