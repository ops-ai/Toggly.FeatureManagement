...
const RELEASE_WORKFLOW = path.join(
  REPO_ROOT,
  ".github/workflows/sdk-dotnet-release.yml",
);
...
export function analyzeNugetTrustedPublishing(content) {
  ...
  if (
    !/NuGet\/login@(?:v1(?:\.\d+)?\b|[0-9a-f]{40}\s+#\s*v1\b)/.test(content)
  ) {
  ...
  if (!/user:\s*opsai\b/.test(content)) {
    errors.push(
      "NuGet/login user must be trust-policy creator opsai (not Toggly org)",
    );
  }
...
if (isMain) {
  ...
  if (!result.ok) {
    console.error(
      `NuGet trusted publishing contract failed (${result.errors.length} error(s)):`,
    );
