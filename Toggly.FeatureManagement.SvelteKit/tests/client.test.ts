...
it('hydrates synchronously, preserves entity gates and composes gate options', () => {
  ...
  expect(
    toggly.isEnabled('Order', { entity: { kind: 'Order', key: 'vip', attributes: { Vip: true } } }),
  ).toBe(true);
...
it('owns its snapshot and local gate notifications, ignores navigation after dispose and avoids transport during SSR', async () => {
  let local = false;
  const initial = { definitions: { on: true }, context: {}, expose: ['on'] };
  const t = createToggly(initial, {
    localGates: [{ id: 'device', flagKeys: ['on'], isEnabled: () => local }],
  });
  initial.definitions.on = false;
  expect(t.isEnabled('on')).toBe(false);
  local = true;
  t.notifyLocalGatesChanged();
  expect(t.isEnabled('on')).toBe(true);
  t.update({ definitions: { off: true }, context: { identity: 'bob' }, expose: ['off'] });
  expect(t.isEnabled('on')).toBe(false);
  await t.start();
  t.dispose();
  t.update(initial);
  await t.start();
  expect(t.isEnabled('off')).toBe(true);
});
