...
`
  ...
  for (const mode of ['conditional', 'legacy-module', 'legacy-cjs', 'ci-dist-overlay', 'ci-dist-overlay-esm']) {
    ...
    try {
      ...
      assert.equal(result.rejected, Object.keys(der.invalid).length + 1)
      console.log(`${mode}: packed webpack browser bundle verifies P1363/DER and rejects tampering/malformed DER`)
    } catch (error) {
      failures += 1
