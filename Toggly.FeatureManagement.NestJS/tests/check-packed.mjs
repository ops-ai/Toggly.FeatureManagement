...
function validateNpmCli(candidate) {
  ...
  try {
    ...
    throw new Error(`npm CLI is unavailable: ${candidate}`);
  }
  ...
  let metadata;
  try {
    metadata = JSON.parse(readFileSync(join(npmRoot, 'package.json'), 'utf8'));
  } catch {
    throw new Error(`npm CLI package metadata is unavailable: ${npmRoot}`);
  }
  if (
    metadata.name !== 'npm' ||
...
try {
  ...
  if (process.env.TOGGLY_TEST_EVAL_TARBALL)
    dependencies['@ops-ai/toggly-eval'] = `file:${resolve(
      process.env.TOGGLY_TEST_EVAL_TARBALL,
    )}`;
  writeFileSync(
  ...
  const core = JSON.parse(
    readFileSync(join(consumer, 'node_modules/@ops-ai/toggly-node-core/package.json'), 'utf8'),
  );
  console.log(
    JSON.stringify({
      packedAdapter: JSON.parse(packed.stdout)[0].name,
      core: core.version,
      nest: nestVersion,
