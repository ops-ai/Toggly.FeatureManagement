import { createParamDecorator, Inject, Injectable, Scope, SetMetadata, type PipeTransform } from '@nestjs/common'
...
export interface FeatureFlagMetadata extends FeatureFlagOptions { keys: string[] }
...
export function FeatureFlag(keys: string | string[], options: FeatureFlagOptions = {}) {
  ...
  if (!normalized.length || normalized.some(key => !key.trim())) throw new Error('FeatureFlag requires non-empty feature keys')
  return SetMetadata(FEATURE_FLAG_METADATA, { keys: normalized, ...options } satisfies FeatureFlagMetadata)
}
...
export class FeatureEnabledPipe implements PipeTransform<string, Promise<boolean>> {
  ...
  transform(key: string): Promise<boolean> { return this.toggly.isFeatureOn(key) }
}
...
export function FeatureEnabled(key: string): ParameterDecorator { return featureKeyParameter(key, FeatureEnabledPipe) }
