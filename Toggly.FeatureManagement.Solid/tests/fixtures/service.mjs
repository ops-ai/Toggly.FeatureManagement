...
const kid =
  createHash('sha1')
    .update(Buffer.concat([Buffer.from(jwk.x, 'base64url'), Buffer.from(jwk.y, 'base64url')]))
    .digest('hex')
    .toUpperCase() + 'ES256';
...
export function envelope(defs, encoding = 'der', timestamp = Math.floor(Date.now() / 1000)) {
  ...
  return JSON.stringify({
    ...
    timestamp,
    kid,
    signature: sign('sha256', digest, { key: privateKey, dsaEncoding: encoding }).toString(
      'base64',
    ),
  });
}
export const backendDefinitions = [
  ...
  {
    featureKey: 'BetaDashboard',
    filters: [
      {
        name: 'Targeting',
        parameters: { 'Audience.Users:0': 'alice', 'Audience.DefaultRolloutPercentage': 0 },
      },
    ],
  },
  {
    featureKey: 'GroupTargeting',
    filters: [
      {
        name: 'Targeting',
        parameters: { 'Audience.Groups:0': 'staff', 'Audience.DefaultRolloutPercentage': 0 },
      },
    ],
  },
  {
    featureKey: 'ClaimsTargeting',
    filters: [
      { name: 'UserClaims', parameters: { Percentage: 100, Claim: 'role', Value: 'admin' } },
    ],
  },
  {
    ...
    filters: [
      { name: 'AlwaysOn', parameters: {} },
      {
        ...
        parameters: { Property: 'Vip', Operator: 'eq', Value: 'true', ValueType: 'boolean' },
      },
    ],
  },
];
export async function startService() {
  ...
  const server = createServer((req, res) => {
    ...
    const payload =
      !backend && state.malformed
        ? { ...defs, ExpressCheckout: { requirement: 'all', rules: [{ property: 'Vip' }] } }
        : defs;
