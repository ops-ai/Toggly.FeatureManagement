...
const jwk = { ...publicKey, kid: createHash('sha1').update(Buffer.from(publicKey.x!, 'base64url')).update(Buffer.from(publicKey.y!, 'base64url')).digest('hex').toUpperCase() + 'ES256', alg: 'ES256' };
async function signed(defs: unknown) {
 ...
 const signature = await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, pair.privateKey, digest);
 return JSON.stringify({ defs, timestamp, kid: jwk.kid, signature: Buffer.from(signature).toString('base64') });
}
const rule = { property: 'Vip', op: 'eq', value: 'true', type: 'boolean' };
const valid = { requirement: 'all', rules: [rule] };
const malformed = [
 ['null map', null], ['array map', []], ['primitive map', true],
 ['nonboolean flag', { Order: 'enabled' }], ['null gate', { Order: null }], ['array gate', { Order: [] }],
...
it.each(malformed)('rejects a signed server payload with %s before serialization', async (_name, defs) => {
 const body = await signed(defs);
 vi.stubGlobal('fetch', vi.fn(async (input: any) => new Response(String(input).endsWith('/.well-known/jwks') ? JSON.stringify({ keys: [jwk] }) : body)));
 const client = createTogglyClient();
 await client.init();
 const onError = vi.fn();
 const event = { locals: {}, request: new Request('http://localhost'), url: new URL('http://localhost') } as any;
 try {
  await createTogglyHandle({ client, frontend: { appKey: 'frontend', expose: ['on', 'Order'], featureDefaults: { on: true, private: true }, onError } })({ event, resolve: async () => new Response() } as any);
  expect((await loadToggly(event)).definitions).toEqual({ on: true });
  expect(onError).toHaveBeenCalledOnce();
 } finally { await client.close(); }
});
it.each(malformed)('retains browser LKG and revision for a signed payload with %s', async (_name, defs) => {
 ...
 let body = goodBody;
 let revision = 'good';
 const fetcher = vi.fn(async (input: any) => new Response(String(input).endsWith('/.well-known/jwks') ? JSON.stringify({ keys: [jwk] }) : body, { headers: { ETag: revision } }));
 vi.stubGlobal('fetch', fetcher);
 const onError = vi.fn();
 const t = createToggly({ definitions: { on: false }, context: {}, expose: ['on', 'Order'] }, { appKey: 'frontend', refreshInterval: 0, onError });
 try {
  await t.start();
  await vi.waitFor(() => expect(t.isEnabled('Order', { entity: order })).toBe(true));
  body = badBody; revision = 'malformed';
...
it('accepts the full supported signed entity contract through server and browser boundaries', async () => {
 ...
 const defs = { ...Object.fromEntries(cases.map((entry, i) => [`gate${i}`, { requirement: i % 2 ? 'any' : 'all', rules: [entry] }])), empty: { requirement: 'all', rules: [] }, on: true, off: false };
 const body = await signed(defs);
 vi.stubGlobal('fetch', vi.fn(async (input: any) => new Response(String(input).endsWith('/.well-known/jwks') ? JSON.stringify({ keys: [jwk] }) : body)));
 const client = createTogglyClient();
 await client.init();
 ...
 let snapshot;
 try {
  await createTogglyHandle({ client, frontend: { appKey: 'frontend', expose: Object.keys(defs) } })({ event, resolve: async () => new Response() } as any);
  snapshot = await loadToggly(event);
  expect(snapshot.definitions).toEqual(defs);
 } finally { await client.close(); }
 const entity = { ...order, attributes: { ...order.attributes, Name: 'alice', Amount: 2, Roles: ['admin'], When: '2025-01-01T00:00:00Z', '': '' } };
 vi.stubGlobal('window', {});
 const t = createToggly({ ...snapshot, definitions: {} }, { appKey: 'frontend', enableLiveUpdates: false, refreshInterval: 0 });
 try {
  await t.start();
