...
{
    [Fact]
    public async Task ModuleIsLazySharedAcrossVerifierAndStorageAndDisposed()
    {
        ...
        js.Setup<ClientSnapshot?>("load", _ => true)
            .SetResult(new("context", "signed", "revision"));
        js.SetupVoid("save", _ => true).SetVoidResult();
        await using var module = new BrowserModule(JSInterop.JSRuntime);
        var verifier = new BrowserSignatureVerifier(module);
        var storage = new BrowserSnapshotStore(module);
    ...
    [Fact]
    public async Task UnusedModuleDoesNotImportOnDisposal()
    {
        await using var module = new BrowserModule(JSInterop.JSRuntime);
    ...
    [Fact]
    public async Task BrowserRegistrationResolvesScopedPortableSessionsWithoutTrustedDependencies()
    {
        Services.AddSingleton(new HttpClient());
        Services.AddTogglyBlazorWebAssembly(_ =>
            new()
            {
        ...
        Assert.True(session.RequiresBrowser);
        await session.InitializeAsync();
        Assert.True(await session.EvaluateAsync(["on"]));
        await session.RefreshAsync();
        EventHandler<Exception> callback = (_, _) => { };
        session.Error += callback;
        session.Error -= callback;
        Assert.Throws<ArgumentNullException>(() =>
            new ServiceCollection().AddTogglyBlazorWebAssembly(null!)
        );
    }
}
