...
public sealed class ServerFeatureSession : IFeatureSession
{
    ...
    )
    {
        ...
        try
        {
            ...
            foreach (var key in keys)
                results.Add(
                    entity is null
                        ? await manager.IsEnabledAsync(key)
                        : await manager.IsEnabledAsync(
                            key,
                            new TogglyEvaluationContext(
                                new TogglyEntityContext(entity.Kind, entity.Key, entity.Attributes)
                            )
                        )
                );
            var result =
                results.Count == 0
                || (requirement == Requirement.Any ? results.Any(x => x) : results.All(x => x));
    ...
    public ValueTask DisposeAsync()
    {
        ...
        if (states is not null)
            states.UnregisterDefinitionsChange(subscription);
        Changed = null;
        Error = null;
        return ValueTask.CompletedTask;
    }
