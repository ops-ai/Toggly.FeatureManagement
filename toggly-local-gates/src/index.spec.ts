import {
  applyLocalGate,
  applyLocalGatesToMap,
  buildFlagGateIndex,
  isLocalPrerequisiteMet,
  LocalGate,
} from './index';

describe('toggly-local-gates', () => {
  const apiRedesignGate: LocalGate = {
    id: 'apiRedesign',
  ...
  describe('buildFlagGateIndex', () => {
    ...
    it('throws when a flag key is registered on multiple gates', () => {
      const other: LocalGate = {
        id: 'other',
        flagKeys: ['ApiV2Checkout'],
        isEnabled: () => true,
      };

      expect(() => buildFlagGateIndex([apiRedesignGate, other])).toThrow(
        /multiple local gates/,
      );
    });
  });
  ...
  describe('applyLocalGate', () => {
    ...
    it('ANDs remote true with local gate when gated', () => {
      const gateOn: LocalGate = { ...apiRedesignGate, isEnabled: () => true };
      const gateOff: LocalGate = { ...apiRedesignGate, isEnabled: () => false };
