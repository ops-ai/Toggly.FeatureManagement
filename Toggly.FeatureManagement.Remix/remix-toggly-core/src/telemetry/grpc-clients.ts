 ...
 */
export function resolveProtoRoot(moduleUrl: string = thisModuleUrl): string {
...
function tryLoadGrpcModules(): { grpc: GrpcJs; protoLoader: ProtoLoader } | null {
  try {
    ...
    const grpc = nodeRequire('@grpc/grpc-js') as GrpcJs
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const protoLoader = nodeRequire('@grpc/proto-loader') as ProtoLoader
