...
### Added
- `readCacheLruIndex` / `writeCacheLruIndex` persist the sidecar LRU index as
  `cache_lru.json` for `TogglyConfig.maxCacheKeys` eviction
...
### Added
- `DiskCacheProvider` now implements `TogglyRevisionCacheProvider`, persisting
