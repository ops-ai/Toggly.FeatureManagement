...
class TogglyClient(TelemetryClientMixin, DefinitionRefreshMixin):
    ...
    def get_variant(
        ...
        *,
        ...
        Precedence when the feature has variants + allocation configured:
        disabled → only ``DefaultWhenDisabled``; enabled → User → Group →
        Percentile → ``DefaultWhenEnabled``. Returns ``None`` when the feature
        is unknown, has no variants defined, or no variant name resolves.

        Args:
            feature_key: Feature key.
            user_id: Targeting user id. Defaults to the client's current
                identity (``set_identity`` / config ``identity``) when omitted.
    ...
    def _fetch_definitions(
        ...
        planned = self._plan_definitions_http_response(
            response, load_jwks=self._load_or_fetch_jwks
        )
