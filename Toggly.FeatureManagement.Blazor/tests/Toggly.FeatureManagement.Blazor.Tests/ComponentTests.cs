...
public class ComponentTests : BlazorTestContext
{
    ...
    [Fact]
    public void NegationMultiKeysAndEntityAreForwardedAndChildContentIsEnabledFallback()
    {
        ...
        var cut = Render<Feature>(p =>
            p.Add(x => x.Keys, new[] { "a", "b" })
                .Add(x => x.Requirement, Requirement.Any)
                .Add(x => x.Negate, true)
                .Add(x => x.Entity, entity)
                .AddChildContent("child")
        );
        Assert.Equal("child", cut.Markup);
        Assert.Equal(new[] { "a", "b" }, session.Keys);
        Assert.True(session.Negate);
        Assert.Equal(Requirement.Any, session.Requirement);
        Assert.Same(entity, session.Entity);
        cut.Render(p => p.Add(x => x.Keys, null));
...
public sealed class FakeSession : IFeatureSession
{
    ...
    )
    {
        ...
        Negate = negate;
        Entity = entity;
        return Evaluate?.Invoke() ?? Task.FromResult(Value);
    }
