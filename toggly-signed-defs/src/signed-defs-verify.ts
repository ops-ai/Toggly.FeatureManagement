...
export function parseSignedEnvelope(bodyText: string): {
  ...
  const defsRaw =
    extractRawJsonProperty(bodyText, 'defs') ??
...
function toP1363Signature(signature: Uint8Array): Uint8Array {
...
export async function computeKid(x: string, y: string): Promise<string> {
 ...
 */
export async function verifySignedDefinitions(
  defsRaw: string,
  envelope: Pick<SignedEnvelope, 'signature' | 'timestamp' | 'kid'>,
  jwks: JwkSet,
  allowedKids?: ReadonlyArray<string> | null,
