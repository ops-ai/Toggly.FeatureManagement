...
test('packed host binds concurrent contexts, guards actions and hydrates exact signed entity snapshots', async ({
  ...
  const [a, b] = await Promise.all([
    request.get('/server?user=alice'),
    request.get('/server?user=bob'),
  ]);
  ...
  expect((await b.json()).on).toBe(false);
  expect((await request.get('/denied')).status()).toBe(404);
  expect(
    (
      await request.post('/one?user=bob&/submit', { form: {}, headers: { origin: baseURL! } })
    ).status(),
  ).toBe(404);
  expect(
    (
      await request.post('/one?user=alice&/submit', { form: {}, headers: { origin: baseURL! } })
    ).status(),
  ).toBe(200);
  ...
  await page.addInitScript(() => {
    ...
    (window as any).togglyFetches = 0;
    window.fetch = (...args) => {
      if (String(args[0]).includes('/evaluated-signed/')) (window as any).togglyFetches++;
      return fetch(...args);
    };
    (window as any).wrongFlash = false;
    new MutationObserver(() => {
      if (document.querySelector('[data-testid="off"]')) (window as any).wrongFlash = true;
  ...
  await expect(page.getByTestId('negate')).toBeVisible();
  await expect
  ...
  for (const [i, message] of [
    ...
    await request.post(definitions + '/control', {
      data: {
        enabled,
        message,
        revision: typeof message === 'object' && 'etag' in message ? 'r2' : 'r1',
      },
    });
    try {
  ...
  for (const shape of [
    { requirement: 'all', rules: [{ property: 'Vip' }] },
    { requirement: 'all', rules: {} },
  ]) {
    const failures = Number(await page.getByTestId('refresh-errors').textContent());
    await request.post(definitions + '/control', {
      data: { shape, enabled: false, revision: 'malformed', message: 'update' },
    });
    await expect
  ...
  await request.post(definitions + '/control', {
    data: { shape: null, invalid: true, message: 'update' },
  });
  await expect
    .poll(async () => Number(await page.getByTestId('refresh-errors').textContent()))
    .toBeGreaterThan(failures);
  ...
  await request.post(definitions + '/control', {
    data: { offline: false, enabled: true, revision: 'r5', delayUser: 'alice', message: 'update' },
  });
  await expect
  ...
  const evaluated = state.requests.filter((r: any) => r.path.startsWith('/evaluated-signed/'));
  expect(evaluated.some((r: any) => r.pin === 'r2' && r.revision === null)).toBe(true);
  expect(evaluated.some((r: any) => r.claims === 'admin' && r.groups.includes('staff'))).toBe(true);
