...
if (process.argv[2]) {
  ...
  const client = createToggly(
    { definitions: {}, context: { identity: 'alice' }, expose: ['on'], source: 'defaults' },
    {
      appKey: 'front',
      storage,
  ...
  try {
    ...
    console.log(
      'Built SvelteKit package restores signed definitions in a new process with every network request disabled.',
    );
  } finally {
