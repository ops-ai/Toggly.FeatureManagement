...
__export(index_exports, {
  HEADERS: () => import_remix_toggly_core3.HEADERS,
  STORAGE_KEYS: () => import_remix_toggly_core3.STORAGE_KEYS,
  TOGGLY_LOADER_KEY: () => import_remix_toggly_core3.TOGGLY_LOADER_KEY,
  TogglyConfigError: () => import_remix_toggly_core3.TogglyConfigError,
  TogglyError: () => import_remix_toggly_core3.TogglyError,
  TogglyNetworkError: () => import_remix_toggly_core3.TogglyNetworkError,
  TogglyServerClient: () => TogglyServerClient,
  TogglyTimeoutError: () => import_remix_toggly_core3.TogglyTimeoutError,
  createFeatureGatedAction: () => createFeatureGatedAction,
  createServerClient: () => createServerClient,
  createTogglyAction: () => createTogglyAction,
  createTogglyLoader: () => createTogglyLoader,
  getFeatureFlags: () => getFeatureFlags,
  isFeatureEnabled: () => isFeatureEnabled2,
  requireFeature: () => requireFeature
});
module.exports = __toCommonJS(index_exports);
var import_remix_toggly_core3 = require("@ops-ai/remix-toggly-core");

// src/client.ts
var import_remix_toggly_core = require("@ops-ai/remix-toggly-core");
var TogglyServerClient = class {
  constructor(config) {
    this.flags = {};
    this.hooks = [];
    this.initialized = false;
    this.config = (0, import_remix_toggly_core.mergeConfig)(config);
    this.logger = (0, import_remix_toggly_core.createLogger)(this.config.debug ?? false);
  ...
  async fetchFlags(identity) {
    ...
    try {
      const url = (0, import_remix_toggly_core.buildDefinitionsUrl)(this.config, identity);
      this.logger.debug(`Fetching flags from: ${url}`);
      const response = await (0, import_remix_toggly_core.fetchWithTimeout)(url, {}, this.config.timeout);
      if (!response.ok) {
        throw new import_remix_toggly_core.TogglyNetworkError(
  ...
  async isEnabled(featureKey, _context, defaultValue = false) {
    ...
    const result = (0, import_remix_toggly_core.isFeatureEnabled)(this.flags, featureKey, defaultValue);
  ...
  async evaluateGate(featureKeys, requirement = "all", negate = false, defaultValue = false) {
    ...
    const result = (0, import_remix_toggly_core.evaluateFeatureGate)(
      this.flags,
      featureKeys,
      requirement,
      negate,
...
// src/loader.ts
var import_remix_toggly_core2 = require("@ops-ai/remix-toggly-core");
function createTogglyLoader(options) {
  ...
  return {
    ...
    async getLoaderData(args, additionalData) {
      ...
      return {
        ...additionalData,
        [import_remix_toggly_core2.TOGGLY_LOADER_KEY]: context
      };
    },
    /**
     * Check if a feature is enabled
...
function getIdentityFromRequest(request, customCookieParser) {
  const headerIdentity = request.headers.get(import_remix_toggly_core2.HEADERS.IDENTITY);
  if (headerIdentity) {
    return (0, import_remix_toggly_core2.parseIdentity)(headerIdentity);
  }
  ...
  if (cookies) {
    const identity = parseCookie(cookies, import_remix_toggly_core2.STORAGE_KEYS.IDENTITY);
    if (identity) {
      return (0, import_remix_toggly_core2.parseIdentity)(identity);
    }
  }
  return void 0;
}
    ...
    if (options.requiredFeatures) {
      ...
      if (!isAllowed) {
        ...
        if (options.redirectTo) {
          return (0, import_server_runtime.redirect)(options.redirectTo);
        }
