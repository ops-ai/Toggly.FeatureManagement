...
describe('Edge Cases & Error Handling', () => {
  ...
  describe('Configuration Edge Cases', () => {
    ...
    it('should handle appKey without environment', () => {
      ...
      const service = new Toggly().init({ appKey: 'test-key' });
      expect(service).toBeTruthy();
    });

    it('should handle re-init with different config', async () => {
      mockFetch.mockRejectedValue(new Error('no net'));
      const s1 = new Toggly().init({ featureDefaults: { F1: true } });
      expect(await s1.isFeatureOn('F1')).toBe(true);

      const s2 = new Toggly().init({ featureDefaults: { F1: false, F2: true } });
  ...
  describe('Hook Edge Cases', () => {
    ...
    it('should handle adding and removing hooks dynamically', () => {
      const service = new Toggly().init({ featureDefaults: {} });
