...
describe('ws-sync', () => {
  describe('buildWebSocketUrl', () => {
    it('appends rev and sdk query params when cached etag exists', () => {
      expect(buildWebSocketUrl('https://definitions.toggly.io', 'app-key', 'abc123'))
        .toBe('wss://definitions.toggly.io/app-key/ws?rev=abc123&sdk=react&sdkVersion=1.12.0');
    });

    it('appends sdk query params when no cached etag', () => {
      expect(buildWebSocketUrl('https://definitions.toggly.io', 'app-key', null))
        .toBe('wss://definitions.toggly.io/app-key/ws?sdk=react&sdkVersion=1.12.0');
    });

    it('converts http to ws', () => {
      expect(buildWebSocketUrl('http://localhost:8787/', 'key', 'rev1'))
        .toBe('ws://localhost:8787/key/ws?rev=rev1&sdk=react&sdkVersion=1.12.0');
    });
  });
  ...
  describe('applyFlagsUpdatedPlan', () => {
    it('routes refresh-jwks to refreshJwks hook', () => {
      const hooks = {
        refreshJwks: jest.fn(),
    ...
    it('routes refresh-pinned to refreshPinned with pin', () => {
      ...
      applyFlagsUpdatedPlan(
        { action: 'refresh-pinned', pin: 'rev-2' },
        { type: 'flags-updated', etag: 'rev-2' },
        hooks,
      );
      expect(hooks.refreshPinned).toHaveBeenCalledWith('rev-2');
      expect(hooks.cacheEtagIfPresent).not.toHaveBeenCalled();
    });

    it('caches etag on none when present', () => {
      const hooks = {
        refreshJwks: jest.fn(),
        refreshPinned: jest.fn(),
        cacheEtagIfPresent: jest.fn(),
