...
describe('verified persistent storage', () => {
  it('retires in-flight responses before the key-rotation debounce window', async () => {
    ...
    const fetcher: typeof fetch = async input => {
      ...
      if (++definitionsRequests === 2) return new Promise<Response>(resolve => { release = resolve; });
    ...
    try {
      ...
      expect(cache.read('https://definitions.toggly.io/evaluated-signed/front/Production', {}, 0)).toBeUndefined();
  ...
  it('restores into a fresh client with every network request disabled', async () => {
    ...
    const fetcher: typeof fetch = async input => {
      ...
      return new Response(String(input).includes('.well-known') ? JSON.stringify(jwks) : envelope({ on: true }));
    };
    ...
    for (const changed of [{ identity: 'bob' }, { environment: 'Other' }, { appKey: 'other' }, { groups: ['staff'] }, { claims: { role: 'admin' } }, { baseURI: 'https://other.test' }]) {
  ...
  it('checks current key pins and rejects changed envelope bytes on restart', async () => {
    ...
    await expect(cache.read('scope', { allowedKeyIds: ['different-key'] }, 0)).rejects.toThrow('not allowed');
  ...
  it('disables persistence for the session when key retirement cannot be stored', () => {
    const storage = memoryStorage();
    let denied = false;
    const guarded = { getItem: storage.getItem, setItem: (key: string, value: string) => {
  ...
  it('ignores absent, corrupt, wrong-scope and unsupported records', () => {
    ...
    for (const value of ['invalid JSON', 'null', '{}', JSON.stringify({ version: 2 }), JSON.stringify({ version: 1, scope: 'other' })]) {
    ...
    for (const mutation of [{ version: 2 }, { scope: 'other' }, { generation: 'other' }, { body: null }, { jwks: null }]) {
      storage.setItem('toggly:solid:envelope:scope', JSON.stringify({ ...JSON.parse(raw), ...mutation }));
      expect(cache.read('scope', {}, 0)).toBeUndefined();
    }
  ...
  it('ignores unavailable storage without attempting to write or read disabled persistence', () => {
    ...
    const denied = createPersistence({ getItem: () => { throw new Error('denied'); }, setItem: () => { throw new Error('denied'); } }, 'https://fixture.test');
...
describe('restored key and envelope validation', () => {
  it.each([
    { kty: 'RSA' }, { crv: 'P-384' }, { alg: 'none' }, { x: 1 }, { y: null },
    { kid: 4 }, { d: 'private' }, { use: 'enc' }, { key_ops: ['sign'] },
    { key_ops: 'verify' }, { exp: 1 }, { exp: 'tomorrow' }, { x: 'changed' },
  ])('rejects invalid key metadata %j', async mutation => {
    await expect(verifyEnvelope(envelope({ on: true }), { keys: [{ ...jwks.keys[0], ...mutation }] } as any, {})).rejects.toThrow();
  });
  ...
  it('accepts constrained public keys and honors expiry and maximum signature age', async () => {
    ...
    expect((await verifyEnvelope(envelope({ on: true }), keys, {})).definitions).toEqual({ on: true });
    await expect(verifyEnvelope(envelope({ on: true }, 'der', now - 120), keys, { maxSignatureAgeSeconds: 60 })).rejects.toThrow('maxSignatureAgeSeconds');
    await expect(verifyEnvelope(envelope({ on: true }, 'der', now + 61), keys, {})).rejects.toThrow('timestamp');
    await expect(verifyEnvelope(envelope({ on: true }, 'der', now), keys, {}, now + 1)).rejects.toThrow('timestamp');
    await expect(verifyEnvelope(envelope({ malformed: { rules: [{ property: 'x' }] } }), keys, {})).rejects.toThrow('Invalid evaluated');
    await expect(verifyEnvelope(envelope({ on: true }), { keys: [] }, {})).rejects.toThrow('Missing');
  });

  it('retains newer active state when an older signed remote envelope arrives', async () => {
    const now = Math.floor(Date.now() / 1000);
    let body = envelope({ on: true }, 'der', now);
    const fetcher: typeof fetch = async input => new Response(String(input).includes('.well-known') ? JSON.stringify(jwks) : body);
